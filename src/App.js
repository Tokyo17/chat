    ...
    <div>
      ...
      <div style={{display:'flex'}}>
        ...
        <div className="bingkai">
          {loadListChat?<p>loading</p> : kontakFrom.map(v=>{
            return  <div key={v.id}> 
              <h5 onClick={()=>{setTo(v.from==userName?v.to:v.from)}} style={{marginBottom:'0px',cursor:'pointer'}}>{v.from==userName?v.to:v.from}</h5>
              <p onClick={()=>{setTo(v.from==userName?v.to:v.from)}} style={{margin:'0px',cursor:'pointer'}}>{v.text} </p>
            </div>
